...
class PCDetROS(Node):
    ...
    def __runTorch__(self, points):
        ...
        with torch.no_grad():
            ...
            pred_dicts, _, _ = self.__net__.forward(data_dict)

            torch.cuda.synchronize()
            
            if 'WBF' in pred_dicts[0]:
                types = pred_dicts[0]['pred_labels'].cpu().numpy()
                scores = pred_dicts[0]['pred_scores'].cpu().numpy()
                boxes_lidar = pred_dicts[0]['pred_boxes'].cpu().numpy()
                types, scores, boxes_lidar = model_nms_utils.compute_WBF(types, scores, boxes_lidar)
            else:
